...
function Calendar({ events, onSelectDate }: MCalendar) {
  ...
  const handleSelect = (e: React.MouseEvent<HTMLElement, MouseEvent>, formattedDate: string) => {
    let mod: any = { slcdate: "", events: [] };
    if (formattedDate) {
      mod.slcdate = formattedDate;
      if (groupedEvents[formattedDate]) mod.events = groupedEvents[formattedDate];
    }
    setSelectedDate(mod);
    e.stopPropagation();
  };
  ...
  return (
    ...
    >
      {weekdays.map((day, index) => (
        <motion.article className="mx-1 py-1" key={`mcalendar-day${index}`}>
          {day}
        </motion.article>
      ))}
      {bufferDays.map((bfd) => (
        <motion.article key={bfd + "_empty"} className="mcalendar-date empty">
          &nbsp;
        </motion.article>
      ))}
      {daysInTheMonth.map((date) => {
        ...
        return (
          <motion.article
            ...
            className={`mcalendar-date d-flex flex-column mx-1 py-1 ${
              selectedDate.slcdate === formattedDate ? "selected" : ""
            }`}
